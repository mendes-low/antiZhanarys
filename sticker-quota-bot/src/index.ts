...
const DEFAULT_ACTION = (process.env.DEFAULT_ACTION || 'delete') as StickerAction;
const DEFAULT_RESTRICT_MINUTES = Number(process.env.DEFAULT_RESTRICT_MINUTES || 60);
...
function shouldUseSsl(url: string): boolean {
  return /render\.com|railway\.internal|railway\.app/i.test(url) || process.env.PGSSL === 'true';
}
  ...
  await pool.query(
    `
      ...
      DO UPDATE SET
        ...
        updated_at = NOW()
    `,
    [chatId, title ?? null, DEFAULT_STICKER_LIMIT, DEFAULT_TIMEZONE, DEFAULT_ACTION, DEFAULT_RESTRICT_MINUTES],
  );
}
...
async function updateGroupSettings(
  chatId: number,
  patch: Partial<Pick<GroupSettings, 'sticker_limit' | 'timezone' | 'action' | 'restrict_minutes' | 'enabled'>>,
): Promise<void> {
...
async function getUserStickerCount(chatId: number, userId: number, dayKey: string): Promise<number> {
...
async function incrementStickerCount(params: {
  ...
  const result = await pool.query<{ sticker_count: number }>(
    ...
    `,
    [params.chatId, params.userId, params.dayKey, params.firstName ?? null, params.username ?? null],
  );

  return result.rows[0].sticker_count;
}

async function userIsPrivileged(chatId: number, userId: number): Promise<boolean> {
...
bot.command('setstickerlimit', async (ctx) => {
  ...
  await ensureGroup(ctx.chat.id, 'title' in ctx.chat ? ctx.chat.title : undefined);
  await updateGroupSettings(ctx.chat.id, { sticker_limit: limit });
  await ctx.reply(`Done. Daily sticker limit is now ${limit}.`);
});
...
bot.command('setstickeraction', async (ctx) => {
  ...
  if (action !== 'delete' && action !== 'restrict') {
    await ctx.reply('Usage: /setstickeraction delete\nUsage: /setstickeraction restrict');
    return;
  }
...
bot.command('mystickers', async (ctx) => {
  ...
  await ctx.reply(`Today you used ${count}/${settings.sticker_limit} stickers in this group.`);
});
...
bot.command('leaderboard', async (ctx) => {
  ...
  const result = await pool.query<{ first_name: string | null; username: string | null; sticker_count: number }>(
    `
  ...
  if (result.rows.length === 0) {
    await ctx.reply('No stickers counted yet today. A peaceful day — suspicious, but peaceful.');
    return;
  }

  const lines = result.rows.map((row, index) => {
    const label = row.username ? `@${row.username}` : row.first_name || 'Unknown user';
...
bot.on(message('sticker'), async (ctx) => {
  ...
  if (settings.action === 'restrict' && ctx.chat.type === 'supergroup') {
    const untilDate = Math.floor(Date.now() / 1000) + settings.restrict_minutes * 60;
